...
if [[ ! -s "${source_archive}" || "$(basename "${source_archive}")" != "kfaceauth-5.1.0.tar.gz" ]]; then
    echo "Expected a non-empty kfaceauth-5.1.0.tar.gz source archive" >&2
    exit 1
fi
...
while IFS= read -r candidate; do
    if [[ "$(rpm -qp --queryformat '%{NAME}' "${candidate}")" == "kfaceauth" &&
          "$(rpm -qp --queryformat '%{VERSION}' "${candidate}")" == "5.1.0" &&
...
if [[ ${#binary_rpms[@]} -ne 1 ]]; then
    echo "Expected exactly one binary kfaceauth 5.1.0 RPM, found ${#binary_rpms[@]}" >&2
    exit 1
fi

mapfile -t source_rpms < <(
    find "${rpmbuild_root}/SRPMS" -maxdepth 1 -type f -name 'kfaceauth-5.1.0-*.src.rpm' -print
)
if [[ ${#source_rpms[@]} -ne 1 || ! -s "${source_rpms[0]:-}" ]]; then
    echo "Expected exactly one non-empty kfaceauth 5.1.0 source RPM" >&2
    exit 1
fi
